...
def grayscale():
    ...
    old_img = img
    old_img.save("static/img/img_old.jpg")
...
def zoomin():
    ...
    if is_grey_scale("static/img/img_now.jpg"):  # Grayscale image
        img_arr = np.pad(img_arr, ((0, 0), (50, 0)), 'constant')[:, 50:]
        new_arr[::2, ::2, 0] = img_arr  # Expand grayscale pixel values
        new_arr[1::2, 1::2, 0] = img_arr
    else:
        r = img_arr[:, :, 0]
        g = img_arr[:, :, 1]
        b = img_arr[:, :, 2]
...
def gaussianblur(size):
    ...
    copyfile("static/img/img_now.jpg", "static/img/img_old.jpg")
    gaussian_blur = cv2.GaussianBlur(src=image, ksize=(size, size), sigmaX=0)
...
def medianblur(size):
    ...
    image = cv2.imread("static/img/img_now.jpg")
    copyfile("static/img/img_now.jpg", "static/img/img_old.jpg")
    medianblur = cv2.medianBlur(src=image, ksize=size)
...
def blurwithsize(size):
    image = cv2.imread("static/img/img_now.jpg")
    copyfile("static/img/img_now.jpg", "static/img/img_old.jpg")
    cv_blur = cv2.blur(src=image, ksize=(size,size))
    cv2.imwrite("static/img/img_now.jpg", cv_blur)

def highPassFilterwithgeneratekernel(size):
    image = cv2.imread("static/img/img_now.jpg")
    copyfile("static/img/img_now.jpg", "static/img/img_old.jpg")
    kernel = generate_zero_sum_matrix(size, -6, 6)
    highFilterImage = cv2.filter2D(image,-1,kernel)
    cv2.imwrite("static/img/img_now.jpg", highFilterImage)


def generate_zero_sum_matrix(sizes, min_value, max_value):
    while True:
        matrix = np.random.randint(min_value, max_value + 1, size=(sizes, sizes))
        current_sum = np.sum(matrix)
        if current_sum != 0:
            non_zero_indices = np.nonzero(matrix)
            rand_index = np.random.choice(len(non_zero_indices[0]))
            i, j = non_zero_indices[0][rand_index], non_zero_indices[1][rand_index]
            adjustment = -matrix[i][j]
            matrix[i][j] += adjustment
        if np.sum(matrix) == 0:
            print("Generated matrix for high pass filter: ")
            print(matrix)
            return matrix

def generate_random_normalized_matrix(sizes):
    while True:
        random_matrix = np.random.rand(sizes, sizes)
        if np.all(random_matrix != 0):
            normalized_matrix = random_matrix / np.sum(random_matrix)
            print("Generated matrix for low pass filter: ")
            print(normalized_matrix)
            return normalized_matrix


def lowPassFilterwithgeneratekernel(size):
    image = cv2.imread("static/img/img_now.jpg")
    copyfile("static/img/img_now.jpg", "static/img/img_old.jpg")
    kernel = generate_random_normalized_matrix(size)
    lowFilterImage = cv2.filter2D(image,-1,kernel)
    cv2.imwrite("static/img/img_now.jpg", lowFilterImage)

def generate_nonzero_integer_matrix(sizes, min_val, max_val):
    while True:
        random_matrix = np.random.randint(min_val, max_val + 1, size=(sizes, sizes))
        if np.sum(random_matrix != 0):
            print("Generated matrix for band pass filter: ")
            print(random_matrix)
            return random_matrix

def bandPassFilterwithgeneratekernel(size):
    image = cv2.imread("static/img/img_now.jpg")
    copyfile("static/img/img_now.jpg", "static/img/img_old.jpg")
    kernel = generate_nonzero_integer_matrix(size, -4, 4)
    bandFilterImage = cv2.filter2D(image,-1,kernel)
    cv2.imwrite("static/img/img_now.jpg", bandFilterImage)
